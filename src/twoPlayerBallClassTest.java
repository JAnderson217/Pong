...
public class twoPlayerBallClassTest {
	//ball coordinates
	private int x;
	private int y;
	...
	public twoPlayerBallClassTest(int x, int y, int ballSpeed, int radius) {
		this.x = x;
		this.y = y;
...
//	}
	...
	public int ballHits(Paddle paddleOne, Paddle paddleTwo, boolean twoPlayers) {
		System.out.println("current speed X: " + getSpeed()[0] + " Y: " + getSpeed()[1]);
		int temp = ballWallHit(paddleOne);
		if (temp == -1) {
			return temp;
		}
		temp = ballPaddleHit(paddleTwo, twoPlayers);
		return ballPaddleHit(paddleOne, twoPlayers);
	}

	public int ballPaddleHit(Paddle paddle, boolean twoPlayers) {
    	...
    	if ((this.x-this.radius*constant == paddle.getCoords()[0] +20*constant) && (this.y-this.radius < paddle.getCoords()[1] + 80 -5) && (this.y+this.radius > paddle.getCoords()[1] +5)) {
    		//Increase speed every other hit
    		//ballSpeedX will always be negative if hits paddle
    		System.out.println("paddle hit");
    		score++;
    		if ((this.score % 2 == 0) && this.ballSpeedX < 0) {
    			this.ballSpeedX -= 1;
    		}
    		else if ((this.score % 2 == 0) && this.ballSpeedX > 0) {
    			this.ballSpeedX += 1;
    		} 
    		...
    		//play sound 
    		try {
				//Sounds.BallPaddleSound();
			} catch (Exception e) {
			}
    		return score;
    	}
    	else if ((this.x-this.radius*constant == paddle.getCoords()[0] +20*constant) && (this.y-this.radius < paddle.getCoords()[1] + 80) && (this.y+this.radius > paddle.getCoords()[1])) {
    		System.out.println("paddle hit");
    		score++;
    		//Increase speed every other hit
    		if ((this.score % 2 == 0)) {
    	...
    	//top side
    	else if (((this.x-this.radius*constant)*constant < (paddle.getCoords()[0]+20*constant)*constant) &&(this.x-this.radius > paddle.getCoords()[0]) && (this.y+this.radius == paddle.getCoords()[1])) {  		
    		System.out.println("paddle hit");
    		this.ballSpeedY =-this.ballSpeedY;
    		score++;
    	}
    	//bottom side
    	else if (((this.x-this.radius*constant)*constant < (paddle.getCoords()[0]+20*constant)*constant) &&(this.x-this.radius > paddle.getCoords()[0]) && (this.y-this.radius == paddle.getCoords()[1] + 80)) {
    		System.out.println("paddle hit");
    		this.ballSpeedY =-this.ballSpeedY;
    		score++;
    	}
